...
param(
    ...
    [string]$AzureFeed,
    [string]$UncachedFeed,
    [System.Security.SecureString]$FeedCredential,
    [string]$ProxyAddress,
...
$FeedCredentialPlain = ""
if ($null -ne $FeedCredential) {
    $FeedCredentialPlain = (New-Object System.Management.Automation.PSCredential("user", $FeedCredential)).GetNetworkCredential().Password
}
...
function Measure-Action($name, $block) {
    ...
    Write-VerboseMsg "Action '$name' took $totalSeconds seconds"
}
...
function Get-Remote-File-Size($zipUri) {
    try {
        ...
        Write-VerboseMsg "Content-Length header was not extracted for $zipUri."
    }

    return $null
}

function Write-InvocationMsg($Invocation) {
    $command = $Invocation.MyCommand;
    $arguments = (($Invocation.BoundParameters.Keys | ForEach-Object { "-$_ `"$($Invocation.BoundParameters[$_])`"" }) -join " ")
    Write-VerboseMsg "$command $arguments"
}
...
function Get-Machine-Architecture() {
    Write-InvocationMsg $MyInvocation
...
function ValidateFeedCredential([string] $FeedToken) {
    if ($Internal -and [string]::IsNullOrWhitespace($FeedToken)) {
    ...
    if ((![string]::IsNullOrWhitespace($FeedToken)) -and ($FeedToken[0] -ne '?')) {
        $FeedToken = "?" + $FeedToken
    }
...
function Get-NormalizedChannel([string]$Channel) {
    ...
    if ($Channel.Contains("Current")) {
        Write-WarningMsg 'Value "Current" is deprecated for -Channel option. Use "STS" instead.'
    }

    if ($Channel.StartsWith('release/')) {
        Write-WarningMsg 'Using branch name with -Channel option is no longer supported with newer releases. Use -Quality option with a channel in X.Y format instead, such as "-Channel 5.0 -Quality Daily."'
    }
...
function Test-TarAvailable {
    if ($env:DOTNET_INSTALL_SKIP_TAR -eq "1") {
        Write-VerboseMsg "Skipping tar detection due to DOTNET_INSTALL_SKIP_TAR environment variable."
        return $false
    }
        ...
        if ($majorVersion -ge 11) {
            ...
            if ($VersionOrChannel -match 'preview\.(\d+)') {
                ...
                if ($majorVersion -eq 11 -and $previewNum -lt 3) {
                    Write-VerboseMsg "Version '$VersionOrChannel' predates tar.gz availability; using zip."
                    return ".zip"
                }
            }
            Write-VerboseMsg "Using tar.gz archive format for version/channel '$VersionOrChannel'."
            return ".tar.gz"
        }
    }
    return ".zip"
...
function GetHTTPResponse([Uri] $Uri, [bool]$HeaderOnly, [bool]$DisableRedirect, [bool]$DisableFeedCredential) {
    ...
    $downloadScript = {
        ...
        try {
            ...
            Import-Assembly -Assembly System.Net.Http
            ...
            if (-not $ProxyAddress) {
                try {
                    ...
                    $ProxyAddress = $null
                    Write-VerboseMsg("Exception ignored: $_.Exception.Message - moving forward...")
                }
            }
            ...
            if ($DisableFeedCredential) {
                ...
                $UriWithCredential = "${Uri}${FeedCredentialPlain}"
            }
...
function Get-Version-From-LatestVersion-File([string]$AzureFeed, [string]$Channel) {
    ...
    Write-VerboseMsg "Constructed latest.version URL: $VersionFileUrl"

    try {
        $Response = GetHTTPResponse -Uri $VersionFileUrl
    }
    catch {
        Write-VerboseMsg "Failed to download latest.version file."
        throw
    }
...
function Get-JsonfileVersion([string]$JSonFile) {
    ...
    try {
        ...
        Write-ErrorMsg "Json file unreadable: '$JSonFile'"
        throw
    }
    if ($JSonContent) {
        try {
            $JSonContent.PSObject.Properties | ForEach-Object {
                ...
                if ($PropertyName -eq "version") {
                    $Version = $_.Value
                    Write-VerboseMsg "Version = $Version"
                }
            }
        }
        catch {
            Write-ErrorMsg "Unable to parse the SDK node in '$JSonFile'"
            throw
        }
    }
    else {
...
function Get-Specific-Version-From-Version([string]$AzureFeed, [string]$Channel, [string]$Version, [string]$JSonFile) {
    ...
    if (-not $JSonFile) {
        ...
        return Get-JsonfileVersion $JSonFile
    }
}
...
function Get-Download-Link([string]$AzureFeed, [string]$SpecificVersion, [string]$CLIArchitecture) {
    ...
    Write-VerboseMsg "Constructed primary named payload URL: $PayloadURL"
...
function Get-LegacyDownload-Link([string]$AzureFeed, [string]$SpecificVersion, [string]$CLIArchitecture) {
    ...
    Write-VerboseMsg "Constructed legacy named payload URL: $PayloadURL"
...
function Get-Product-Version([string]$AzureFeed, [string]$SpecificVersion, [string]$PackageDownloadLink) {
    ...
    Foreach ($ProductVersionTxtURL in $ProductVersionTxtURLs) {
        Write-VerboseMsg "Checking for the existence of $ProductVersionTxtURL"
        ...
        try {
            ...
            if ($productVersionResponse.StatusCode -eq 200) {
                ...
                Write-VerboseMsg "Got StatusCode $($productVersionResponse.StatusCode) when trying to get productVersion.txt at $productVersionTxtUrl."
            }
        } 
        catch {
            Write-VerboseMsg "Could not read productVersion.txt at $productVersionTxtUrl (Exception: '$($_.Exception.Message)'. )"
        }
    }

    # Getting the version number with productVersion.txt has failed. Try parsing the download link for a version number.
    if ([string]::IsNullOrEmpty($PackageDownloadLink)) {
        Write-VerboseMsg "Using the default value '$SpecificVersion' as the product version."
...
function Get-Product-Version-Url([string]$AzureFeed, [string]$SpecificVersion, [string]$PackageDownloadLink, [bool]$Flattened) {
    ...
    Write-VerboseMsg "Constructed productVersion link: $ProductVersionTxtURL"
...
function Get-ProductVersionFromDownloadLink([string]$PackageDownloadLink, [string]$SpecificVersion) {
    ...
    if ($filenameParts.Length -gt 2) {
        ...
        Write-VerboseMsg "Extracted product version '$productVersion' from download link '$PackageDownloadLink'."
    }
    else {
        Write-VerboseMsg "Using the default value '$SpecificVersion' as the product version."
...
function Get-User-Share-Path() {
    ...
    elseif ($InstallRoot -like "$env:ProgramFiles\dotnet\?*") {
        Write-WarningMsg "The install root specified by the environment variable DOTNET_INSTALL_DIR points to the sub folder of $env:ProgramFiles\dotnet which is the default dotnet install root using .NET SDK installer. It is better to keep aligned with .NET SDK installer."
    }
    return $InstallRoot
}
...
function Test-DotnetPackageInstalled([string]$InstallRoot, [string]$RelativePathToPackage, [string]$SpecificVersion) {
    ...
    Write-VerboseMsg "Test-DotnetPackageInstalled: DotnetPackagePath=$DotnetPackagePath"
...
function Get-List-Of-Directories-And-Versions-To-Unpack-From-Dotnet-Package([System.IO.Compression.ZipArchive]$Zip, [string]$OutPath) {
    ...
    Write-VerboseMsg "Directories to unpack: $values"
...
function Expand-DotnetPackage([string]$ZipPath, [string]$OutPath) {
    ...
    Import-Assembly -Assembly System.IO.Compression.FileSystem
    ...
    try {
        ...
        Write-ErrorMsg "Failed to extract package. Exception: $_"
        throw;
    }
    finally {
        if ($null -ne $Zip) {
...
function Expand-DotnetPackage-Tar([string]$TarPath, [string]$OutPath) {
    Write-InvocationMsg $MyInvocation
    ...
    foreach ($dir in $versionedDirs.GetEnumerator()) {
        if ($dir.Value) {
            ...
            $excludeArgs += '--exclude'
            $excludeArgs += $pattern
            Write-VerboseMsg "Excluding pre-existing versioned directory: $($dir.Key)"
        }
    }
        ...
        foreach ($entry in $tarListing) {
            ...
            if (-not $match.Success) {
                ...
                if (Test-Path $localPath) {
                    ...
                    Write-VerboseMsg "Excluding pre-existing non-versioned file: $normalizedEntry"
                }
            }
        }
    }
    ...
    try {
        ...
        Write-ErrorMsg "Failed to extract tar package. Exception: $_"
        throw
    }
}
...
function SafeRemoveFile($Path) {
    try {
        if (Test-Path $Path) {
            Remove-Item $Path
            Write-VerboseMsg "The temporary file `"$Path`" was removed."
        }
        else {
            Write-VerboseMsg "The temporary file `"$Path`" does not exist, therefore is not removed."
        }
    }
    catch {
        Write-WarningMsg "Failed to remove the temporary file: `"$Path`", remove it manually."
    }
}

function Update-SdkInstallRootPath([string]$InstallRoot) {
    ...
    if (-Not $NoPath) {
        ...
        if (-Not $env:path.Contains($SuffixedBinPath)) {
            ...
            Write-VerboseMsg "Current process PATH already contains `"$BinPath`""
        }
    }
    else {
...
function Get-AkaMSDownloadLink([string]$Channel, [string]$Quality, [bool]$Internal, [string]$Product, [string]$Architecture, [string]$FileExtension = ".zip") {
    ...
    if (![string]::IsNullOrEmpty($Quality) -and (@("LTS", "STS") -contains $Channel)) {
        $Quality = ""
        Write-WarningMsg "Specifying quality for STS or LTS channel is not supported, the quality will be ignored."
    }
    Write-VerboseMsg "Retrieving primary payload URL from aka.ms link for channel: '$Channel', quality: '$Quality' product: '$Product', os: 'win', architecture: '$Architecture'." 
    ...
    Write-VerboseMsg  "Constructed aka.ms link: '$akaMsLink'."
    ...
    for ($maxRedirections = 9; $maxRedirections -ge 0; $maxRedirections--) {
        ...
        Write-VerboseMsg "Received response:`n$Response"

        if ([string]::IsNullOrEmpty($Response)) {
            Write-VerboseMsg "The link '$akaMsLink' is not valid: failed to get redirect location. The resource is not available."
            return $null
        }
        ...
        if ($Response.StatusCode -eq 301) {
            try {
                ...
                if ([string]::IsNullOrEmpty($akaMsDownloadLink)) {
                    Write-VerboseMsg "The link '$akaMsLink' is not valid: server returned 301 (Moved Permanently), but the headers do not contain the redirect location."
                    return $null
                }

                Write-VerboseMsg "The redirect location retrieved: '$akaMsDownloadLink'."
                # This may yet be a link to another redirection. Attempt to retrieve the page again.
                $akaMsLink = $akaMsDownloadLink
                continue
            }
            catch {
                Write-VerboseMsg "The link '$akaMsLink' is not valid: failed to get redirect location."
                return $null
            }
        }
        ...
        Write-VerboseMsg "The link '$akaMsLink' is not valid: failed to retrieve the redirection location."
        return $null
    }

    Write-VerboseMsg "Aka.ms links have redirected more than the maximum allowed redirections. This may be caused by a cyclic redirection of aka.ms links."
    return $null
...
function Get-AkaMsLink-And-Version([string] $NormalizedChannel, [string] $NormalizedQuality, [bool] $Internal, [string] $ProductName, [string] $Architecture) {
    ...
    foreach ($ext in $extensionsToTry) {
        ...
        Write-VerboseMsg "aka.ms link resolution with '$ext' extension failed, trying next format."
    }
   
    if ([string]::IsNullOrEmpty($AkaMsDownloadLink)) {
        if (-not [string]::IsNullOrEmpty($NormalizedQuality)) {
            # if quality is specified - exit with error - there is no fallback approach
            Write-ErrorMsg "Failed to locate the latest version in the channel '$NormalizedChannel' with '$NormalizedQuality' quality for '$ProductName', os: 'win', architecture: '$Architecture'."
            Write-ErrorMsg "Refer to: https://aka.ms/dotnet-os-lifecycle for information on .NET Core support."
            throw "aka.ms link resolution failure"
        }
        Write-VerboseMsg "Falling back to latest.version file approach."
        return ($null, $null, $null)
    }
    else {
        Write-VerboseMsg "Retrieved primary named payload URL from aka.ms link: '$AkaMsDownloadLink'."
        Write-VerboseMsg  "Downloading using legacy url will not be attempted."

        #get version from the path
        $pathParts = $AkaMsDownloadLink.Split('/')
        if ($pathParts.Length -ge 2) { 
            $SpecificVersion = $pathParts[$pathParts.Length - 2]
            Write-VerboseMsg "Version: '$SpecificVersion'."
        }
        else {
            Write-ErrorMsg "Failed to extract the version from download link '$AkaMsDownloadLink'."
            return ($null, $null, $null)
        }
        ...
        Write-VerboseMsg "Product version: '$EffectiveVersion'."
...
function New-InstallDirectory {
    ...
    if ($PSVersionTable.PSVersion.Major -lt 7) {
        Write-VerboseMsg $diskSpaceWarning
        return
    }
    ...
    try {
        ...
        Write-WarningMsg $diskSpaceWarning
    }
...
Write-VerboseMsg "Note that the intended use of this script is for Continuous Integration (CI) scenarios, where:"
Write-VerboseMsg "- The SDK needs to be installed without user interaction and without admin rights."
Write-VerboseMsg "- The SDK installation doesn't need to persist across multiple CI runs."
Write-VerboseMsg "To set up a development environment or to run apps, use installers rather than this script. Visit https://dotnet.microsoft.com/download to get the installer.`r`n"
...
Measure-Action "Product discovery" {
    ...
    Write-VerboseMsg "Normalized quality: '$NormalizedQuality'"
    $script:NormalizedChannel = Get-NormalizedChannel $Channel
    Write-VerboseMsg "Normalized channel: '$NormalizedChannel'"
    $script:NormalizedProduct = Get-NormalizedProduct $Runtime
    Write-VerboseMsg "Normalized product: '$NormalizedProduct'"
    $script:FeedCredentialPlain = ValidateFeedCredential $FeedCredentialPlain
    $script:TarAvailable = Test-TarAvailable
    Write-VerboseMsg "Tar available: '$TarAvailable'"
}

$InstallRoot = Resolve-Installation-Path $InstallDir
if (-not (Test-User-Write-Access $InstallRoot)) {
    Write-ErrorMsg "The current user doesn't have write access to the installation root '$InstallRoot' to install .NET. Please try specifying a different installation directory using the -InstallDir parameter, or ensure the selected directory has the appropriate permissions."
    throw
}
Write-VerboseMsg "InstallRoot: $InstallRoot"
...
if ([string]::IsNullOrEmpty($JSonFile) -and ($Version -eq "latest")) {
    ...
    if ($null -ne $DownloadLink) {
        ...
        Write-VerboseMsg "Generated aka.ms link $DownloadLink with version $EffectiveVersion"
        
        if (-Not $DryRun) {
            Write-VerboseMsg "Checking if the version $EffectiveVersion is already installed"
            if (Test-DotnetPackageInstalled -InstallRoot $InstallRoot -RelativePathToPackage $dotnetPackageRelativePath -SpecificVersion $EffectiveVersion) {
                Say "$assetName with version '$EffectiveVersion' is already installed."
                Update-SdkInstallRootPath -InstallRoot $InstallRoot
                return
            }
        }
    }
...
if ([string]::IsNullOrEmpty($NormalizedQuality) -and 0 -eq $DownloadLinks.count) {
    foreach ($feed in $feeds) {
        try {
            ...
            Write-VerboseMsg "Generated primary link $DownloadLink with version $EffectiveVersion"
            ...
            if (-not [string]::IsNullOrEmpty($LegacyDownloadLink)) {
                ...
                Write-VerboseMsg "Generated legacy link $LegacyDownloadLink with version $EffectiveVersion"
            }
            ...
            Write-VerboseMsg "Failed to acquire download links from feed $feed. Exception: $_"
        }
    }
}
...
Measure-Action "Installation directory preparation" { New-InstallDirectory }

Write-VerboseMsg "Zip path: $ZipPath"
...
foreach ($link in $DownloadLinks) {
    Write-VerboseMsg "Downloading `"$($link.type)`" link $($link.downloadLink)"

    try {
        Measure-Action "Package download" { DownloadFile -Source $link.downloadLink -OutPath $ZipPath }
        Write-VerboseMsg "Download succeeded."
        ...
        Write-VerboseMsg "Download failed with status code $StatusCode. Error message: $ErrorMessage"
...
if ($DownloadedLink.downloadLink.EndsWith(".tar.gz")) {
    Measure-Action "Package extraction" { Expand-DotnetPackage-Tar -TarPath $ZipPath -OutPath $InstallRoot }
}
else {
    Measure-Action "Package extraction" { Expand-DotnetPackage -ZipPath $ZipPath -OutPath $InstallRoot }
}
...
if ($DownloadedLink.effectiveVersion -Match "rtm" -or $DownloadedLink.effectiveVersion -Match "servicing") {
    ...
    Write-VerboseMsg "Checking installation: version = $ReleaseVersion"
    $isAssetInstalled = Test-DotnetPackageInstalled -InstallRoot $InstallRoot -RelativePathToPackage $dotnetPackageRelativePath -SpecificVersion $ReleaseVersion
}

#  Check if the SDK version is installed.
if (!$isAssetInstalled) {
    Write-VerboseMsg "Checking installation: version = $($DownloadedLink.effectiveVersion)"
    $isAssetInstalled = Test-DotnetPackageInstalled -InstallRoot $InstallRoot -RelativePathToPackage $dotnetPackageRelativePath -SpecificVersion $DownloadedLink.effectiveVersion
}

# Version verification failed. More likely something is wrong either with the downloaded content or with the verification algorithm.
if (!$isAssetInstalled) {
    Write-ErrorMsg "Failed to verify the version of installed `"$assetName`".`nInstallation source: $($DownloadedLink.downloadLink).`nInstallation location: $InstallRoot.`nReport the bug at https://github.com/dotnet/install-scripts/issues."
...
Measure-Action "Setting up shell environment" { Update-SdkInstallRootPath -InstallRoot $InstallRoot }
